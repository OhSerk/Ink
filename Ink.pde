...
String parola[], grid[], colors[], fill[], diff[], record[], col[], visual[], datiutili[], lingue[], nomeGiocatore[], 
  tastiera[] = {"Q", "W", "E", "R", "T", "Y", "U", "I", "O", "P", "A", "S", "D", "F", "G", "H", "J", "K", "L", "Z", "X", "C", "V", "B", "N", "M"};
static int lol = 0; //Uso in buttonone
...
public void setup() {
  ...
  File nom = new File(dataPath("nomegiocatore.txt"));
  if (nom.exists())nomeGiocatore = loadStrings(dataPath("nomegiocatore.txt")); 
  else { 
    nomeGiocatore = new String[2];
    nomeGiocatore[0] = nomeGiocatore[1] = "";
  }
  scene = parola[19];
  noStroke();
  textAlign(CENTER);
  ...
  background(255);
  mouseY = height/10;
  if (musica) parti(aptheme); //faccio partire la musica
  endsetup=true;
...
public void draw() {
  ...
  if (scene.equals("logo")) {
    ...
    nondevi = true;
    text(parola[19], 180, 40, 30, colore(6), 6);
    text(parola[31]+nomeGiocatore[1], 180, 120, 25, colore(4), 4);
    text("< "+parola[27]+": "+parola[28]+" >", 180, 220, 30, colore(2), 2);
    for (int i = 0; i < Key.length; i++)
      Key[i].view();
    if (nomeGiocatore[1].equals("")) text("Inserisci un nome diverso!", 180, 550, 30, colore(1), 1);
    else text("Premi '"+parola[19]+"' per continuare", 180, 550, 20, colore(3), 3);
    if (rel && mouseY >= 180*height/640 && mouseY <= 260*height/640) scene = "x";
  } else if (scene.equals("x")) {
    cambialingua();
  } else if (scene.equals(parola[31])) {
    background(0);
    nondevi = true;
    text(parola[19], 180, 40, 30, colore(6), 6);
    text(parola[31]+nomeGiocatore[1], 180, 120, 25, colore(4), 4);
    for (int i = 0; i < Key.length; i++)
      Key[i].view();
    if (nomeGiocatore[1].equals("")) text(parola[32], 180, 550, 30, colore(1), 1);
    else text(parola[33]+"'"+parola[19]+"'"+parola[34], 180, 550, 20, colore(3), 3);
  }
...
public void menu() {
  background(0);
  fill(255);
  text("Ciao "+nomeGiocatore[1]+"!", 180, 40, 25);
...
public void creazioneGriglia(boolean uno) {
  ...
  size = PApplet.parseInt(sqrt((sq(s.length)+sq(s[0].length))*pow(PApplet.parseInt(colors[k]), 1)));
...
public void gioco_1() {
  ...
  fill(255);
  if (nMosse <= size)
    text(parola[18]+""+nMosse+"/"+size, 270, 100, 27);
  text(parola[17]+visual[n], 80, 100, 27);
  text(parola[19], 180, 40, 30, colore(6), 6);
}
...
public void gameover(boolean win) {
  ...
  for (int i = 0; i < 6; i++) //Stampo i quadrati del gameover
    if ((i <= 2 || singleplayer) && (i != 3 || (n > 0 || k > 0)) && (i != 5 || (n != grid.length-1 || k != colors.length-1))) {
...
public void text(String a, float x, float y, float ts, int fil, int i) {
  ...
  if (mouseY >= (y-ts*1.5f)*height/640 && mouseY <= (y+ts/2)*height/640) { 
    ...
    tint(0);
    cont = true;
    if (scene.equals(parola[1]) && a.equals(parola[31])) {
      scene = a;
    } else if (rel && (scene.equals(parola[19]) || a.equals(parola[19]))) {
      boolean flag = true;
      flag = !nomeGiocatore[1].equals("");
      if (rel && (scene.equals(parola[1]) || scene.equals(parola[31]) || scene.equals("Linguaggiamelo")) && flag) {
        nondevi = false;
        if (!scene.equals(parola[31])) {
          datiutili = new String[4];
        ...
        salvataggio("nomegiocatore.txt", nomeGiocatore);
